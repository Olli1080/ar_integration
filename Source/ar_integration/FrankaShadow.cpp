...
void UFrankaShadowController::Tick(float DeltaSeconds)
{
...
void UFrankaShadowController::set_visual_plan(const TArray<F_joints_synced>& plan)
{
	last_update_ = FDateTime::UtcNow() + look_ahead;
...
TOptional<F_joints_synced> UFrankaShadowController::current_sync_joints() const
{
	if (done())
		return {};
...
TOptional<FFrankaJoints> UFrankaShadowController::current_joints_interp() const
{
...
void UFrankaShadowController::set_robot(AFranka* franka)
{
	franka_ = franka;
}

bool UFrankaShadowController::done() const
{
	return plan_.IsEmpty() || plan_.Last().time_stamp < last_update_;
}

void UFrankaShadowController::clear_Implementation()
{
	plan_.Empty();
}

void UFrankaShadowController::set_visibility_Implementation(Visual_Change vis_change)
{
	switch (vis_change)
	{
	case ENABLED:
...
void UFrankaShadowController::advance_to_present()
{
