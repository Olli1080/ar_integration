...
AIntegrationGameState::AIntegrationGameState()
{
	...
	debug_client = NewObject<UDebugClient>();
	mesh_client = NewObject<UMeshClient>();

	franka_client = NewObject<UFrankaClient>();
	franka_tcp_client = NewObject<UFrankaTcpClient>();
	//franka_joint_client = NewObject<UFrankaJointClient>();
	franka_joint_sync_client = NewObject<UFrankaJointSyncClient>();

	franka_controller_ = NewObject<UFrankaShadowController>();

	channel = NewObject<UGrpcChannel>();
...
void AIntegrationGameState::BeginPlay()
{
	Super::BeginPlay();
	...
	pcl_client = GetWorld()->SpawnActor<APclClient>(params);
	//pcl_client->visualize = false;

	hand_tracking_client = GetWorld()->SpawnActor<A_hand_tracking_client>(params);

	franka_voxel = GetWorld()->SpawnActor<AFrankaVoxel>(params);
	franka_tcps = GetWorld()->SpawnActor<AFrankaTcps>(params);

	franka = GetWorld()->SpawnActor<AFranka>(params);
	franka_controller_->set_robot(franka);

	QuestRegistrationManager = GetWorld()->SpawnActor<AQuestRegistrationManager>(params);
	QuestManager = GetWorld()->SpawnActor<AQuestManager>(params);
	QuestInteractionManager = GetWorld()->SpawnActor<AQuestInteractionManager>(params);
	...
	franka_client->on_voxel_data.AddDynamic(this, &AIntegrationGameState::handle_voxels);
	franka_client->on_visual_change.AddDynamic(franka_voxel, &IFrankaInterface::set_visibility);

	franka_tcp_client->on_tcp_data.AddDynamic(this, &AIntegrationGameState::handle_tcps);
	franka_tcp_client->on_visual_change.AddDynamic(franka_tcps, &IFrankaInterface::set_visibility);

	franka_joint_sync_client->on_sync_joint_data.AddDynamic(this, &AIntegrationGameState::handle_sync_joints);
	franka_joint_sync_client->on_visual_change.AddDynamic(franka_controller_, &IFrankaInterface::set_visibility);

	//franka_joint_client->on_joint_data.AddDynamic(this, &AIntegrationGameState::handle_joints);

	on_post_actors.Broadcast();
}

void AIntegrationGameState::Tick(float DeltaSeconds)
{
...
void AIntegrationGameState::change_channel(FString target, int32 retries)
{
	 ...
	 * execution conflicts
	 */
	IBaseClientInterface::Execute_set_channel(debug_client, channel);
	IBaseClientInterface::Execute_set_channel(franka_client, channel);
	IBaseClientInterface::Execute_set_channel(franka_tcp_client, channel);
	//IBaseClientInterface::Execute_set_channel(franka_joint_client, channel);
	IBaseClientInterface::Execute_set_channel(franka_joint_sync_client, channel);
	IBaseClientInterface::Execute_set_channel(pcl_client, channel);
	/**
	 * create new object_client and bind all the signals to
	 * respective update queues
	 */
	object_client = NewObject<UObjectClient>();
	IBaseClientInterface::Execute_set_channel(object_client, channel);

	object_client->on_object_instance_data.AddDynamic(
		this, &AIntegrationGameState::set_object_instance_data);
	object_client->on_object_instance_colored_box.AddDynamic(
		this, &AIntegrationGameState::set_object_instance_colored_box);
	object_client->on_object_delete.AddDynamic(
		this, &AIntegrationGameState::delete_object);
	
	IBaseClientInterface::Execute_set_channel(mesh_client, channel);
	...
	IBaseClientInterface::Execute_set_channel(hand_tracking_client, channel);
...
void AIntegrationGameState::spawn_obj_proto(const FString& name)
{
	...
	const auto newActor = GetWorld()->SpawnActor<AProceduralMeshActor>(
...
void AIntegrationGameState::set_object_instance_data(
...
void AIntegrationGameState::set_object_instance_colored_box(
...
void AIntegrationGameState::delete_object(const FString& id)
{
	std::unique_lock lock(delete_mutex);
	delete_list.Add(id);
}

void AIntegrationGameState::update_anchor_transform(
...
void AIntegrationGameState::sync_and_subscribe(bool forced)
{
...
void AIntegrationGameState::update_meshes(const TSet<FString>& pending_proto)
{
	/**
...
void AIntegrationGameState::update_actors(const TArray<FString>& to_delete)
{
	/**
...
void AIntegrationGameState::handle_object_instance(const F_object_instance& instance)
{
	FTransform trafo;
	std::function<void(AProceduralMeshActor* actor)> f;
	...
	AProceduralMeshActor* actor = find_or_spawn(id);
	f(actor);
...
void AIntegrationGameState::init()
{
	/**
...
FString AIntegrationGameState::get_object_instance_id(const F_object_instance& data)
{
...
bool AIntegrationGameState::get_prototype_and_mesh(
...
F_procedural_mesh_data AIntegrationGameState::create_proc_mesh_data(
	const F_object_prototype& proto,
	const F_mesh_data& mesh)
{
	return {
...
AProceduralMeshActor* AIntegrationGameState::spawn_mesh_actor(const FString& id)
{
	FActorSpawnParameters spawnParams;
	spawnParams.bNoFail = true;
	
	auto temp = GetWorld()->SpawnActor<AProceduralMeshActor>(
		AProceduralMeshActor::StaticClass(), spawnParams);
	
	return actors.Emplace(id, temp);
}

AProceduralMeshActor* AIntegrationGameState::find_or_spawn(const FString& id)
{
...
void AIntegrationGameState::handle_voxels(const F_voxel_data& data)
{
	//if (!franka_voxel->IsHidden())
		franka_voxel->set_voxels(data);
}

void AIntegrationGameState::handle_tcps(const TArray<FVector>& data)
{
	//if (!franka_tcps->IsHidden())
		franka_tcps->set_tcps(data);
}

void AIntegrationGameState::handle_joints(const FFrankaJoints& data)
{
	franka->SetJoints(data);
}

void AIntegrationGameState::handle_sync_joints(const TArray<F_joints_synced>& data)
{
	//if (!franka->IsHidden())
