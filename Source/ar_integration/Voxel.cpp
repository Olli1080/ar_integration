...
void AVoxel::Tick(float DeltaSeconds)
{
...
void AVoxel::BeginDestroy()
{
	Super::BeginDestroy();
}

void AVoxel::insert(const TArray<FVector>& points)
{
	init = false;
...
//
//void AVoxel::insert_sync(const TArray<FVector>& points)
//{
//	init = false;
//
...
//}
//
//void AVoxel::insert_point(const FVector& point)
//{
//	init = false;
//
//	/**
...
void AVoxel::set_voxel_size(float size)
{
	size = std::abs(size);
	if (init && size > 0.f)
		voxel_size = size;
